...
namespace Engine {
    ...
    Entity PrefabInstantiator::InstantiateScenePrefab(
        ...
        LOG_INFO("PrefabInstantiator: Instantiated scene prefab " , prefab->GetName(), "with", createdEntities.size(), "entities");
   ...
   std::vector<Entity> PrefabInstantiator::CreateEntitiesAndBuildIDMap(
        ...
        Entity& rootEntity)
    {
       LOG_DEBUG("==== Start CreateEntitiesAndBuildIDMap ====");
       ...
       for (rapidjson::SizeType i = 0; i < entitiesArray.Size(); i++) {
           ...
           if (entityObj.HasMember("ID")) {
               ...
               LOG_WARNING("PrefabInstantiator: Entity", i,  "has no ID in JSON");
               continue;
           }
           ...
           if (!newEntity) {
               LOG_WARNING("PrefabInstantiator: Failed to deserialize entity", i , "(old ID : ", oldID, ")");
               continue;
           }
       ...
       LOG_INFO("PrefabInstantiator: Created ", createdEntities.size(), "entities with ID mapping" );
       LOG_DEBUG("==== End CreateEntitiesAndBuildIDMap ====");
   ...
   {
       LOG_DEBUG("==== Start FixTransformHierarchy ====");
       ...
       for (Entity entity : entities) {
           ...
           if (transform.Parent != 0xFFFFFFFF) {
               ...
               if (parentIt != idMapping.end()) {
                   ...
                   LOG_DEBUG("PrefabInstantiator: Entity", entityID, "parent remapped ", transform.Parent, "-> ",
                      newParentID);
                   transform.Parent = newParentID;
               }
               else {
                   LOG_WARNING("PrefabInstantiator: Parent ID ", transform.Parent, "not found in mapping for entity ",
                      entityID);
               }
           }
           ...
           for (uint32_t oldChildID : transform.Children) {
               ...
               if (childIt != idMapping.end()) {
                  ...
                  // LOG_DEBUG("PrefabInstantiator: Entity: ", entityID, "child remapped: ", ,
                   ...
                   LOG_WARNING("PrefabInstantiator: Child ID ", oldChildID, "not found in mapping for entity",
                       entityID);
               }
           }
           ...
           if (newChildren.size() != transform.Children.size()) {
               LOG_INFO("PrefabInstantiator: Entity ", entityID, "has ", newChildren.size(), "children(mapped from)",
                  transform.Children.size());
           }
       }

       LOG_INFO("PrefabInstantiator: Fixed transform hierarchy for ", entities.size() ," entities.");
       LOG_DEBUG("==== End FixTransformHierarchy ====");
   }
   ...
   void PrefabInstantiator::ApplyPrefabComponentToAll(
       ...
       xresource::instance_guid prefabGUID)
   {
       LOG_DEBUG("==== Start ApplyPrefabComponentToAll ====");

       for (Entity entity : entities) {
           if (!entity.HasComponent<PrefabComponent>()) {
               entity.AddComponent<PrefabComponent>(prefabGUID);
               LOG_DEBUG("PrefabInstantiator: Added PrefabComponent (GUID: ", prefabGUID.m_Value, ") to entity ",
                  static_cast<uint32_t>(entity));
           }
       }

       LOG_INFO("PrefabInstantiator: Applied PrefabComponent to {} entities", entities.size());
       LOG_DEBUG("==== End ApplyPrefabComponentToAll ====");
   }
