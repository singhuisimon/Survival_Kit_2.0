...
// Serialize RigidbodyComponent
			...
			if (entity.HasComponent<ScriptComponent>())
			{
				...
				auto &script = entity.GetComponent<ScriptComponent>();  // Get reference
				Value componentObj(kObjectType);
				...
				// Serialize fields if script instance exists
				if (script.ScriptInstance)
				{
					LOG_TRACE("    - Serializing script fields for: ", script.ScriptClassName);
				...
				{
					LOG_WARNING("    - Script instance is null for: ", script.ScriptClassName);
				}
			...
			if (entityObj.HasMember("Components"))
			{
				...
				for (SizeType j = 0; j < components.Size(); j++)
				{
					...
					else if (componentType == "ScriptComponent")
					{
						...
						if (script.ScriptInstance)
						{
							LOG_TRACE("    - Script instance created successfully");

							// Restore field values if they exist
							if (properties.HasMember("Fields"))
							{
								LOG_TRACE("    - Deserializing script fields");
								...
								LOG_TRACE("    - Script fields restored");
							}
							else
							{
								LOG_TRACE("    - No fields to deserialize");
							}
						}
						else
						{
							LOG_ERROR("    - Failed to create script instance for: ", script.ScriptClassName);
						}
					}
