...
namespace Engine
{
	...
	static inline bool NearlyEqual(float a, float b, float eps = 1e-5f) { return std::fabs(a - b) <= eps; }
			...
			[&](EntityID e, TransformComponent &tc, RigidbodyComponent &rb)
			{
				...
				if (mFetchMeshInfo)
				{
					...
					if (mFetchMeshInfo(scene, e, info))
					{
						...
						if (info.key != ap.meshKey || kind != ap.shapeKind || ds != ap.shapeDS || scaleDiff)
						{
							DestroyBodyFor(e);
							CreateBodyFor(scene, e);
							return;
