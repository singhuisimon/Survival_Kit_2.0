...
*/
    ...
    */
    ...
    // TESTING FOR MAIN MENU, IF IT DOSEN'T WORK, SWITCH TO SCRIPTS


    /**
     * @brief Checks if a keyboard key is currently pressed
     * @details Returns Success if the key is pressed, Failure otherwise
     *
     * Common GLFW Key Codes:
     *   SPACE=32, ENTER=257, ESCAPE=256, E=69, F=70
     *   1=49, 2=50, 3=51, etc.
     */
    class BTCheckKeyPressed : public BTNode {
    public:
        BTCheckKeyPressed(int keyCode = 257);  // Default: Enter key

        const char* GetTypeName() const override { return "CheckKeyPressed"; }
        BTStatus Execute(BTContext& context) override;
     ...
     * @details MouseButton: 0=Left, 1=Right, 2=Middle
     */
    class BTCheckMouseButtonPressed : public BTNode {
    public:
        BTCheckMouseButtonPressed(int mouseButton = 0);  // Default: Left click

        const char* GetTypeName() const override { return "CheckMouseButtonPressed"; }
        BTStatus Execute(BTContext& context) override;
    ...
    /**
     * @brief Stores current mouse position in blackboard as vec2
     */
    class BTGetMousePosition : public BTNode {
    public:
        BTGetMousePosition(const std::string& positionKey = "MousePosition");

        const char* GetTypeName() const override { return "GetMousePosition"; }
        BTStatus Execute(BTContext& context) override;
    ...
    /**
     * @brief Enables or disables a camera by entity tag
     * @details Finds entity with CameraComponent matching the tag and sets Enabled flag
     */
    class BTSetCameraEnabled : public BTNode {
    public:
        BTSetCameraEnabled(const std::string& cameraTag = "", bool enabled = true);

        const char* GetTypeName() const override { return "SetCameraEnabled"; }
        BTStatus Execute(BTContext& context) override;

        void GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const override;
        void SetProperty(const std::string& name, const std::string& value) override;

        std::string m_CameraTag;
        bool m_Enabled;
    };

    /**
     * @brief Switches between two cameras in one node
     * @details Disables one camera and enables another - convenient for menu/game transitions
     */
    class BTSwitchCamera : public BTNode {
    public:
        BTSwitchCamera(const std::string& disableTag = "", const std::string& enableTag = "");

        const char* GetTypeName() const override { return "SwitchCamera"; }
        BTStatus Execute(BTContext& context) override;

        void GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const override;
        void SetProperty(const std::string& name, const std::string& value) override;

        std::string m_DisableCameraTag;
        std::string m_EnableCameraTag;
    };
