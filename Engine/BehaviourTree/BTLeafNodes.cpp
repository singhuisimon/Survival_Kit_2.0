...
namespace Engine {
    ...
    // TESTING NODES FOR MAIN MENU UI, IF DOSEN'T WORK, SWITCH TO SCRIPTS


    // --- BTCheckKeyPressed ---
    BTCheckKeyPressed::BTCheckKeyPressed(int keyCode)
        : m_KeyCode(keyCode) {
    }

    BTStatus BTCheckKeyPressed::Execute(BTContext& context) {
        (void)context;

        GLFWwindow* window = glfwGetCurrentContext();
        if (!window) {
            LOG_WARNING("BTCheckKeyPressed: No GLFW window context");
            return BTStatus::Failure;
        }

        int state = glfwGetKey(window, m_KeyCode);
        if (state == GLFW_PRESS) {
            LOG_TRACE("BTCheckKeyPressed: Key ", m_KeyCode, " is pressed");
            return BTStatus::Success;
        }
    ...
    void BTCheckKeyPressed::GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const {
        properties.push_back({ "KeyCode", std::to_string(m_KeyCode) });
    }

    void BTCheckKeyPressed::SetProperty(const std::string& name, const std::string& value) {
        if (name == "KeyCode") {
            m_KeyCode = std::stoi(value);
        }
    }

    // --- BTCheckMouseButtonPressed ---
    BTCheckMouseButtonPressed::BTCheckMouseButtonPressed(int mouseButton)
        : m_MouseButton(mouseButton) {
    }

    BTStatus BTCheckMouseButtonPressed::Execute(BTContext& context) {
        (void)context;

        GLFWwindow* window = glfwGetCurrentContext();
        if (!window) {
            LOG_WARNING("BTCheckMouseButtonPressed: No GLFW window context");
            return BTStatus::Failure;
        }

        int state = glfwGetMouseButton(window, m_MouseButton);
        if (state == GLFW_PRESS) {
            LOG_TRACE("BTCheckMouseButtonPressed: Mouse button ", m_MouseButton, " is pressed");
            return BTStatus::Success;
        }
    ...
    void BTCheckMouseButtonPressed::GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const {
        properties.push_back({ "MouseButton", std::to_string(m_MouseButton) });
    }

    void BTCheckMouseButtonPressed::SetProperty(const std::string& name, const std::string& value) {
        if (name == "MouseButton") {
            m_MouseButton = std::stoi(value);
        }
    }

    // --- BTGetMousePosition ---
    BTGetMousePosition::BTGetMousePosition(const std::string& positionKey)
        : m_PositionKey(positionKey) {
    }

    BTStatus BTGetMousePosition::Execute(BTContext& context) {
        GLFWwindow* window = glfwGetCurrentContext();
        if (!window) {
            LOG_WARNING("BTGetMousePosition: No GLFW window context");
            return BTStatus::Failure;
        }
        ...
        context.Blackboard.Set(m_PositionKey, mousePos);

        LOG_TRACE("BTGetMousePosition: Stored mouse position (", mousePos.x, ", ", mousePos.y, ")");
        return BTStatus::Success;
    }

    void BTGetMousePosition::GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const {
        properties.push_back({ "PositionKey", m_PositionKey });
    }

    void BTGetMousePosition::SetProperty(const std::string& name, const std::string& value) {
        if (name == "PositionKey") {
            m_PositionKey = value;
        }
    }
    ...
    // --- BTSetCameraEnabled ---
    BTSetCameraEnabled::BTSetCameraEnabled(const std::string& cameraTag, bool enabled)
        : m_CameraTag(cameraTag)
        , m_Enabled(enabled) {
    }

    BTStatus BTSetCameraEnabled::Execute(BTContext& context) {
        if (!context.Scene) {
            LOG_WARNING("BTSetCameraEnabled: No scene context");
            return BTStatus::Failure;
        }

        if (m_CameraTag.empty()) {
            LOG_WARNING("BTSetCameraEnabled: Camera tag is empty");
            return BTStatus::Failure;
        }
        ...
        try {
            ...
            for (auto entityHandle : view) {
                ...
                // Check if tag matches (exact match or contains)
                if (tag.Tag == m_CameraTag || tag.Tag.find(m_CameraTag) != std::string::npos) {
                    // Check if entity has CameraComponent
                    if (entity.HasComponent<CameraComponent>()) {
                        auto& camera = entity.GetComponent<CameraComponent>();
                        camera.Enabled = m_Enabled;
                        camera.isDirty = true;
                        found = true;

                        LOG_INFO("BTSetCameraEnabled: Set camera '", tag.Tag, "' Enabled = ", m_Enabled ? "true" : "false");
                    }
                }
            }

            if (!found) {
                LOG_WARNING("BTSetCameraEnabled: No camera found with tag '", m_CameraTag, "'");
                return BTStatus::Failure;
            }
            ...
            LOG_ERROR("BTSetCameraEnabled: Exception - ", e.what());
            return BTStatus::Failure;
        }
    }

    void BTSetCameraEnabled::GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const {
        properties.push_back({ "CameraTag", m_CameraTag });
        properties.push_back({ "Enabled", m_Enabled ? "true" : "false" });
    }

    void BTSetCameraEnabled::SetProperty(const std::string& name, const std::string& value) {
        if (name == "CameraTag") {
            m_CameraTag = value;
        }
    ...
    // --- BTSwitchCamera ---
    BTSwitchCamera::BTSwitchCamera(const std::string& disableTag, const std::string& enableTag)
        : m_DisableCameraTag(disableTag)
        , m_EnableCameraTag(enableTag) {
    }

    BTStatus BTSwitchCamera::Execute(BTContext& context) {
        if (!context.Scene) {
            LOG_WARNING("BTSwitchCamera: No scene context");
            return BTStatus::Failure;
        }
        ...
        try {
            ...
            for (auto entityHandle : view) {
                ...
                // Check if entity has CameraComponent
                if (!entity.HasComponent<CameraComponent>()) {
                    continue;  // Skip entities without camera
                }

                auto& camera = entity.GetComponent<CameraComponent>();

                // Check if this is the camera to disable
                if (!m_DisableCameraTag.empty() &&
                    (tag.Tag == m_DisableCameraTag || tag.Tag.find(m_DisableCameraTag) != std::string::npos)) {
                    camera.Enabled = false;
                    camera.Primary = false;
                    camera.isDirty = true;
                    disabledOne = true;
                    LOG_INFO("BTSwitchCamera: Disabled camera '", tag.Tag, "'");
                }

                // Check if this is the camera to enable
                if (!m_EnableCameraTag.empty() &&
                    (tag.Tag == m_EnableCameraTag || tag.Tag.find(m_EnableCameraTag) != std::string::npos)) {
                    camera.Enabled = true;
                    camera.Primary = true;
                    camera.isDirty = true;
                    enabledOne = true;
                    LOG_INFO("BTSwitchCamera: Enabled camera '", tag.Tag, "'");
                }
            }

            if (!m_DisableCameraTag.empty() && !disabledOne) {
                LOG_WARNING("BTSwitchCamera: No camera found with tag '", m_DisableCameraTag, "'");
            }
            if (!m_EnableCameraTag.empty() && !enabledOne) {
                LOG_WARNING("BTSwitchCamera: No camera found with tag '", m_EnableCameraTag, "'");
            }

            return (disabledOne || enabledOne) ? BTStatus::Success : BTStatus::Failure;
        }
        catch (const std::exception& e) {
            LOG_ERROR("BTSwitchCamera: Exception - ", e.what());
            return BTStatus::Failure;
        }
    }

    void BTSwitchCamera::GetProperties(std::vector<std::pair<std::string, std::string>>& properties) const {
        properties.push_back({ "DisableCameraTag", m_DisableCameraTag });
        properties.push_back({ "EnableCameraTag", m_EnableCameraTag });
    }

    void BTSwitchCamera::SetProperty(const std::string& name, const std::string& value) {
        if (name == "DisableCameraTag") {
            m_DisableCameraTag = value;
        }
        else if (name == "EnableCameraTag") {
            m_EnableCameraTag = value;
        }
    }
