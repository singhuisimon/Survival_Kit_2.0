...
namespace Game
{
    public class Level2Tutorial : ScriptBehaviour
    {
        ...
        private void SpawnTutorialOnWall()
        {
            ...
            // Spawn enemy directly (NO VFX during tutorial)
            uint enemyID = PrefabInstantiate(enemyPrefabPath);
            if (enemyID == 0)
            {
                LogMessage("[Level2Tutorial] Fail to instantiate enemy for: " + enemyPrefabPath);
                return;
            }
            ...
            // Track tutorial enemy IDs and publish events
            if (enemyDex == 0)
            {
                currentBotnetSpawned++;
            ...
            else if (enemyDex == 2)
            {
                ...
                // Publish all tutorial events once all 3 enemies are spawned
